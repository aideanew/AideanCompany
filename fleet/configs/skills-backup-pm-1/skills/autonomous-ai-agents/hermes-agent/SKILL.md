...
**Config path on Windows differs from POSIX.** On Windows, `hermes config path`
reports `C:\Users\<user>\AppData\Local\hermes\config.yaml` (or equivalently
`%LOCALAPPDATA%\hermes\config.yaml`). The `~/.hermes/` POSIX path does NOT
exist on Windows — do NOT assume `read_file("~/.hermes/config.yaml")` will
work. Always use `~/AppData/Local/hermes/config.yaml` or call
`hermes config path` to get the actual path.

**Finding config on Windows.** When searching for config files on Windows:
```python
import os
paths_to_check = [
    os.path.expanduser("~/AppData/Local/hermes/config.yaml"),  # ACTUAL location
    os.path.expanduser("~/.hermes/config.yaml"),              # POSIX - does NOT exist
]
```

### Terminal Tool / Bash Dependency

**Terminal tool requires bash/WSL.** On Windows, the `terminal` tool invokes
`/bin/bash` via WSL relay. If WSL is not installed or `bash` is not on PATH,
the tool fails with `CreateProcessCommon:800: execvpe(/bin/bash) failed: No such file or directory`.

**Workaround for Windows:** Use `execute_code` (Python sandbox) instead of
`terminal` for file operations, path exploration, and running Python scripts.
`execute_code` has better Windows compatibility. For shell commands on Windows
without WSL, use `execute_code` with `subprocess.run(["cmd", "/c", "..."], ...)`.
