...
try:
    from .personas import ExpertPersona
    from .strategies import PromptEnhancer, CognitiveStrategyManager
...
class SupervisorAgent:
    ...
    def register_expert(self, worker_id: str, persona: Any):
        """
        Register a worker agent with a specific Expert Persona.
        
        Args:
            worker_id: The ID of the worker.
            persona: The ExpertPersona object defining the agent.
        """
        capabilities = persona.capabilities if hasattr(persona, 'capabilities') else []
        
        # Use existing register structure but add persona details
        self.workers[worker_id] = {
            "capabilities": capabilities,
            "status": "available",
            "current_task": None,
            "metrics": {"tasks_completed": 0, "avg_response_time": 0},
            "persona": persona,
            # We store the base prompt generation capability
            "prompt_generator": persona.generate_system_prompt if hasattr(persona, 'generate_system_prompt') else None
        }
    ...
    def assign_task(self, subtask: Dict, worker_id: str):
        ...
        # 1. Enhance Input (Basic Scenario)
        original_description = subtask.get("description", "")
        enhanced_description = PromptEnhancer.enhance(original_description) if PromptEnhancer else original_description
        
        # 2. Detect and Apply Strategy (Extended Scenario)
        if worker_info.get("persona") and worker_info.get("prompt_generator"):
            mode = CognitiveStrategyManager.detect_mode(original_description) if CognitiveStrategyManager else None
            subtask["dynamic_system_prompt"] = worker_info["prompt_generator"](
                task_context=enhanced_description,
                mode=mode
            )
        # --------------------------------------
        ...
        self.send(AgentMessage(
            ...
            content={
                "action": "execute_task",
                "task": subtask,
                "enhanced_input": enhanced_description
            },
